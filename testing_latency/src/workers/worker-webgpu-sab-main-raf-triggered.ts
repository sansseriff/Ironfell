...
self.onmessage = (event: MessageEvent) => {
  ...
  switch (data.ty) {
    ...
    case 'start':
      ...
      break;
    case 'render':
      render(data.mainSeq, data.mainRafNow, data.mainAbsRafNow);
      break;
    case 'stop':
      active = false;
      break;
...
async function init(nextCanvas: OffscreenCanvas, nextOptions: TestOptions) {
  ...
  if (!context) throw new Error('Main-RAF-triggered worker WebGPU canvas context is unavailable');
...
function render(mainSeq: number, mainRafNow: number, mainAbsRafNow: number) {
