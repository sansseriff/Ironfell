...
export interface BufferedWorkerWebGpuOptions {
  name: string;
  blockMs?: number;
  renderLagFrames?: number;
}

export class BufferedWorkerWebGPUApp implements LatencyTestApp {
  ...
  constructor(private readonly config: BufferedWorkerWebGpuOptions) {
  ...
  async mount(root: HTMLElement, options: TestOptions): Promise<void> {
    ...
    this.worker = new Worker(new URL('../workers/worker-webgpu-buffered.ts', import.meta.url), { type: 'module' });
    await new Promise<void>((resolve, reject) => {
      ...
      this.worker.postMessage({
        ...
        canvas: offscreen,
        options,
        blockMs: this.config.blockMs || 0,
        renderLagFrames: this.config.renderLagFrames || 0,
      }, [offscreen]);
    });
    this.worker.onmessage = (event) => {
