...
export class Worker2DApp implements LatencyTestApp {
  readonly name = 'worker-2d';
  ...
  async mount(root: HTMLElement, options: TestOptions): Promise<void> {
    ...
    this.worker = new Worker(new URL('../workers/worker2d.ts', import.meta.url), { type: 'module' });
